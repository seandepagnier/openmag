...
void init_VIC(void) 
{
	...
	*vect_cntl = 0;
    ...
    VICDefVectAddr = (long)DefaultVICHandler;   
    return;
}
...
** 
******************************************************************************/
long install_irq( long IntNumber, void *HandlerAddr )
{
    long i;
    long *vect_addr;
    long *vect_cntl;
	...
	vect_addr = (long *)(VIC_BASE_ADDR + VECT_ADDR_INDEX + i*4);
	vect_cntl = (long *)(VIC_BASE_ADDR + VECT_CNTL_INDEX + i*4);
	if ( *vect_addr == (long)NULL )
	{
	    *vect_addr = (long)HandlerAddr;	/* set interrupt vector */
	    *vect_cntl = (long)(IRQ_SLOT_EN | IntNumber);
	    break;
	}
    }
    if ( i == VIC_SIZE )
    {
	return( 0 );		/* fatal error, can't find empty vector slot */
    }
...
** 
******************************************************************************/
long uninstall_irq( long IntNumber )
{
    long i;
    long *vect_addr;
    long *vect_cntl;
	...
	{
	    *vect_addr = (long)NULL;	/* clear the VIC entry in the VIC table */
    ...
    if ( i == VIC_SIZE )
    {
	return( 0 );		/* fatal error, can't find interrupt number 
					in vector slot */
    }
    VICIntEnable = 1 << IntNumber;	/* Enable Interrupt */
    return( 1 );
