...
//
// Copyright (C) 2007 Sean D'Epagnier
// Added support for detecting usb at init, and only enabling the driver
// if there is a host
//
...
{
#ifdef USBTINY_DETECT
        /* turn on 1.5k pullup on D- */
        USB_DDR |= USB_MASK_DETECT;
        USB_OUT |= USB_MASK_DETECT;
#endif
...
/* turn usb off */
#ifdef USBTINY_DETECT
extern void usb_off ( void )
{
        /* disable interrupt */
        USB_INT_ENABLE &= ~(1 << USB_INT_ENABLE_BIT);

        /* turn off 1.5k pullup on D- */
        USB_DDR &= ~USB_MASK_DETECT;
        USB_OUT &= ~USB_MASK_DETECT;
}
#endif
...
extern	void	usb_init ( void )
{
        ...
        USB_OUT |= USB_MASK_DPLUS; /* enable pullup */
        int i;
        for(i=0; i<10; i++); /* wait for decent sample */
        int p = USB_IN & USB_MASK_DPLUS;
        USB_OUT &= ~USB_MASK_DPLUS; /* disable pullup */
        if(!p) /* only start if pullup went low */
                usb_on();
#else
        usb_on();
#endif
